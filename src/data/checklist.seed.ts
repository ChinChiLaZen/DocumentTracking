// Content sourced 2026-07-25 from the two Lovable reference prototypes named in
// CLAUDE.md §1 (pattern-to-page-pal.lovable.app for the 28-item register text,
// check-chime-charm.lovable.app for the 14 detail sheets' rows/columns/tick state).
// This replaces the earlier structurally-correct-but-placeholder seed. The tick
// state below is exactly what the prototypes render — it is prototype seed data,
// not the real Airsafe workbook, but it is real content rather than a placeholder.
...
const ITEM_SOURCE: Record<number, ItemSource> = {
  1: {
...
const FIXTURE_ROWS: { article: string; description: string }[] = [
...
const item6Columns: CheckColumn[] = [{ key: 'dimmingCurve', label: 'Dimming Curve Data' }]
const item7Columns: CheckColumn[] = [{ key: 'corrosionReport', label: 'Corrosion Test Report' }]
