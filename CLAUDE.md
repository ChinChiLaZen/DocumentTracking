...
api/
  ...
  projects/
    [[...path]].ts           # GET /api/projects (list), POST /api/projects/seed (idempotent bulk seed when empty), PUT/DELETE /api/projects/:id (upsert / admin-only delete) — one file, not three, to stay under Vercel Hobby's 12-serverless-function cap (§10)
  _lib/
...
- **Vercel Hobby plan caps a deployment at 12 serverless functions** (one per `api/**/*.ts` route file, excluding `_lib/`) — discovered when adding the 3 new project routes pushed the count to 15 and deployment failed outright. Fixed by folding `api/projects/{index,seed,[id]}.ts` into a single `api/projects/[[...path]].ts` optional catch-all (routes by segment count/value instead of filename) and doing the same for `api/tasks/{index,[id]}.ts` → `api/tasks/[[...id]].ts`, bringing the total back to 12. **Before adding any new `api/**` route file, count existing ones first** — either fold the new route into an existing catch-all file, or consolidate an existing pair the same way to make room.
