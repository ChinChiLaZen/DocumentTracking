...
src/
  ...
  domain/
    ...
    thaiBahtText.ts           # Thai BAHTTEXT-equivalent number-to-words converter (§5.3d's Excel export)
    thaiBahtText.test.ts      # locks in a real ground-truth example from masterfile/BOQ Estimate.xlsx
    thaiDate.ts               # Thai Buddhist-era long-date formatter (§5.3d's Excel export)
  ...
  components/
    ...
    board/                   # Task Board tab (§7, §5.3e) — added, additive, independent of §6
      TaskBoardPage.tsx        # container: stat cards, status breakdown, Add Group, group sections
...
interface BoqLine {
  ...
  remark?: string;             // "หมายเหตุ" column in the Excel export — optional, backward-compat
}
...
```
`ProjectRecord.boq?: BoqEstimate` — optional for backward-compat with rows that predate this field (`toRuntime()`/`hydrate()` default a missing value to `{ categories: [], vatPercent: 7 }`, same posture as `schedule`). Generic project-costing data, **independent of checklist structure** — shown for every `TemplateKind` alike, same posture as Project Management (§5.3c). Modeled on a reference site (`quick-boq-form.lovable.app`, styled after the real ปร.4/ปร.5 Thai government BOQ forms) for **structural format only** — categories-as-tabs, per-line `quantity × (materialUnitCost + laborUnitCost)` math, VAT calc, category-share breakdown — its placeholder item content is never reproduced, so a new project's `boq` always starts as `{ categories: [], vatPercent: 7 }`, the same "start empty" posture as `schedule`'s `{ phases: [], milestones: [] }`. Category/line numbering (`"1.1"`, `"2.3"`) is derived at render time from array order, never stored. `domain/boq.ts` (pure, framework-free, unrelated to §6) provides `lineTotal`/`categoryTotal`/`summarizeBoq` (subtotal, VAT amount, net total, and each category's % share of the pre-VAT subtotal). Add/Edit/Delete category and line, and the VAT % field, are gated to `admin`/`ProjectManager` roles, **client-side only** — same accepted-risk posture as Schedule (§5.3c) and Add/Edit Project (§10); unlike Schedule's per-button gating, the whole BOQ tab is read-only end-to-end for other roles (every numeric input is `disabled={!canEdit}`), not just its Add/Delete actions. No audit trail — same reasoning as §5.3c (`HistoryField`/`HistoryEntry` are keyed by `itemNo` and don't fit category/line edits). `resetToSeed` also clears `boq` back to `{ categories: [], vatPercent: 7 }`, consistent with `schedule`/`history`. Category color (on-screen tab accent + category-share progress bar only) reuses `PHASE_COLOR_SLOTS`/`phaseColorIndex` (`statusStyles.ts`/`domain/schedule.ts`) via `phaseColorIndex(category.id)` — same palette and hashing utility Project Management uses for phases, for a stable per-id color with zero new palette code; **not** a claim that BOQ categories and Schedule phases are the same underlying entity, since the two features have entirely unrelated id spaces.

**Excel export (added 2026-09-05, rewritten to match a real official template)** — `domain/export/boqExcelExport.ts` byte-for-structure matches a real Thai government BOQ template (`masterfile/BOQ Estimate.xlsx`, not committed — a local reference file, ask the user for it again if regenerating), not just the earlier Lovable-prototype structural inspiration above: two sheets ("Overview" then "BOQ"), `TH SarabunPSK` font, black-thin-border-only styling (no fills, matching the source file's own visual language — deliberately does **not** reuse `PHASE_COLOR_SLOTS`/`SCHEDULE_COLORS` here), and a fully formula-driven workbook (`IF(AND(qty<>"",price<>""),qty*price,"")` line amounts, `SUM()` category subtotals, a `+`-joined grand total, and an Overview-sheet cross-sheet reference to the BOQ sheet's own grand-total cell) rather than pre-computed values — Excel recalculates on open, same as the source file. A pure `computeBoqLayout(boq)` (in the same file) precomputes every row number once so the Overview sheet's `=BOQ!I{row}` reference can never drift out of sync with the BOQ sheet's actual grand-total row as category/line counts change; a category with zero lines is omitted from the export entirely but keeps its original 1-based number (so exported numbering always matches on-screen numbering). Two new small pure-math modules support this, reused nowhere else: `domain/thaiBahtText.ts` (a Thai BAHTTEXT-equivalent number-to-words converter — implements the standard digit-reading algorithm including the "เอ็ด"/"ยี่สิบ"/silent-"สิบ" irregulars, locked in by a test asserting the real ground-truth example from the source file: 64,999,169.09 → "หกสิบสี่ล้านเก้าแสนเก้าหมื่นเก้าพันหนึ่งร้อยหกสิบเก้าบาทเก้าสตางค์") and `domain/thaiDate.ts` (a Thai Buddhist-era long-date formatter, written as a plain computed string rather than a locale-dependent Excel numFmt like the source file's own `[$-107041E]d mmmm yyyy;@`, since that numFmt risks rendering as `#####` on a non-Thai-locale Excel install). `meta.scope`→"สถานที่" (location) and `meta.vendor`→"หน่วยงานเจ้าของโครงการ" (owning agency) in the export are disclosed best-effort reuses of existing `ProjectMeta` fields, not new data-model fields — mirrors `scheduleExcelExport.ts`'s own Overview sheet reusing the same two fields. The "ประมาณการโดย" (estimator name) label is left blank in the export — no backing field exists anywhere in this app's data model, and a signed-in user's email is not the same thing as "who prepared this estimate."

### 5.3e Task Board (`taskBoard`, added — Monday.com-style free-form task tracker)

Modeled on a reference site (`careful-code.lovable.app`, a Thai-language project-delivery task tracker) and monday.com generally, for **layout/interaction pattern only** — grouped task sections, colored status/priority pills, milestone-relative due dates, live progress bars/stat cards, auto-save-on-edit. Colors are this app's own existing Tailwind token system (§9), not the reference site's palette.

```ts
type TaskStatus = 'NotStarted' | 'InProgress' | 'PendingReview' | 'Done' | 'Blocked';
type TaskPriority = 'Low' | 'Medium' | 'High' | 'Critical';
type TaskDueDateMode = 'none' | 'fixed' | 'milestone';
...
| `/projects/:projectId/board` | **Task Board** | A Monday.com-style free-form task tracker, layout/interaction modeled on a reference site (`careful-code.lovable.app`) — see §5.3e. User-named groups, each holding ad-hoc tasks (name/assignee/status/priority/due date/progress), entirely independent of the checklist register. Header stat cards (Total/Done/In Progress/Overdue) and a status-breakdown row above the groups; due dates are either a fixed date or relative to a Project Management milestone (`schedule.milestones`, reused not duplicated) with a day offset. Shown for **every** `templateKind`, unlike the MAR-only tabs above. Add/Edit/Delete gated to Admin/Project Manager; the whole tab is read-only for other roles, same posture as BOQ Estimate. |
| `/projects/:projectId/guidelines` | *(unlinked, 2026-09-07)* | Priority definitions, critical sequence, document-quality rules, colour legend, disclaimer (rules.ts). Removed from the tab bar and Quick Navigation per explicit request — the route/`GuidelinesPage.tsx` still exist and render fine if hit directly, same degrade-gracefully posture as the AOT/DOA-unlinked routes below. |
...
- **Header band:** removed 2026-09-07 — the on-screen `ProjectShell` header/tab bar was restyled to match the prototype exactly (plain white/`bg-background` header, no colour band; underline-style text tabs instead of filled pills), applied globally to every `templateKind` (not just MAR), resolving the previously-open reconciliation gap. The `--color-header-band` CSS token (`#0F766E`, live 2026-09-05–2026-09-07) was deleted from `index.css`. Caveat: the Project Management/BOQ Estimate Excel exports' header-row fill (`SCHEDULE_COLORS.headerBg` in `scheduleExcelFormat.ts`) still uses the literal hex `#0F766E` — deliberately left unchanged as an independent, fixed export-format color, no longer described as "matching" any on-screen element now that the on-screen header is no longer teal. The ADS-B Excel/Word exports' navy header (`ADSB_COLORS` in `adsbFormat.ts`) remains untouched for the same pre-existing reason — it matches a real external reference template family, not this app's own branding.
...
- **Task Board badges (Task Board tab, added):** Status — Not Started `slate-100/600` · In Progress `amber-50/700` · Pending Review `sky-50/700` · Done `emerald-50/700` · Blocked `rose-50/700` (`TASK_STATUS_BADGE_CLASS`), reusing `WORKFLOW_STATUS_BADGE_CLASS`'s hue-per-state mapping for the same shaped 5-state pipeline plus rose for the hard-stop Blocked state. Priority — Low `emerald-100/800` · Medium `amber-100/800` · High `rose-100/800` · Critical `violet-100/800` (`TASK_PRIORITY_BADGE_CLASS`), an ascending 4-step escalation reusing `IMPORTANCE_BADGE_CLASS`'s existing rose→violet "critical→more than critical" convention. Both in `statusStyles.ts`. The Status `Select`'s trigger itself carries the badge color (the dropdown *is* the pill) — new to this app, since Task Board is explicitly modeled on Monday.com's colorful clickable status chips, unlike Phase Progress's deliberately calmer plain-`Select` `WorkflowStatus` field.
...
```
- **Schema:** one row per project in `project_records` (`api/_lib/db.ts`) — `id, meta, items, sheets, history` as JSONB, plus `updated_by`/`updated_at`. Project existence = row existence; display order = `ORDER BY seq ASC` (an auto-increment tie-breaker column) — there is no separate `projectOrder` table. `schedule` (§5.3c), `boq` (§5.3d, BOQ Estimate tab), and `task_board` (§5.3e, Task Board tab) were all added later as their own JSONB columns via a self-migrating `ALTER TABLE project_records ADD COLUMN IF NOT EXISTS ... DEFAULT '...'::jsonb` in `ensureSchema()` — no manual migration step, same pattern for all three. `task_board` is the one snake_case, multi-word column name among the three (`schedule`/`boq` are single words needing no translation) — `api/projects/index.ts` explicitly maps it to/from the client's `taskBoard` field.
